...
static int do_read( const char *path, char *buffer, size_t size, off_t offset, struct fuse_file_info *fi )
{
  ...
  short *bmap = load_blockmap();
  short traverse = offset / BLOCK_SIZE;
  if(traverse > 0) {
  	short *map = load_blockmap();
  	while(traverse > 0) {
  		bid = map[bid];
  		traverse--;
  	}
  ...
  // TODO: [READ_OFFSET] account for the offset! May need to traverse the blocks of this file
	...
	memcpy( buffer, &bcache[offset % BLOCK_SIZE], rsize );
...
static int do_rename(const char *opath, const char *npath) {
  ...
  const char* oldpath = &opath[1];
  load_directory();
  int ni = find_dir_entry(oldpath);
  //printf(ni);
  if(ni <0 ) {
  	  printf("error");
  	  return -ENFILE;
  ...
  const char *newpath = &npath[1];
  strncpy(de->name, newpath, FS_NAME_LEN);
  save_directory();
...
static int do_unlink(const char *path) {
  ...
  const char *fn = &path[1];
  load_directory();
  int ni = find_dir_entry(fn);
  if(ni < 0)
  	return -ENFILE;
  int trunc = do_truncate(path, 0);
  if(trunc < 0) {
